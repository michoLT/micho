  ...
  <body>
    ...
    <article>
      ...
      <div class="text">
        ...
        $query = mysqli_query($con_photos,"Select * from photoscivic ORDER BY time DESC");
        ...
        <?php
          if (iMEMBER){
            if (iADMIN){
        ?>
              <form action='gallery-civic.php' method='POST' onclick='return confirmation()'>
        ...
        <?php
              if (isset($_POST['delete_photo'])){
                @extract($_POST);
                mysqli_query($con_photos,"DELETE from photoscivic WHERE id='$id'");
                  if ($id == $row['id']) {
                    echo "you have successfully deleted photo!";
                    header("Refresh:3; url=gallery-civic.php");
                }
              }
            }
          }
